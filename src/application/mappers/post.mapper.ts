...
export class PostMapper implements Mapper<PostDTO, Post> {
	public toDomain(dto: PostDTO): Post {
		return Post.create({
			...
			postLikes: PostVotes.create(dto.postLikes.map(new PostVoteMapper().toDomain)),
	...
	public toPersistence(domain: Post): PostDTO {
		return {
			...
			postLikes: domain.likes.getItems().map(new PostVoteMapper().toPersistence),
			comments: domain.comments.getItems().map(new CommentMapper().toPersistence),
